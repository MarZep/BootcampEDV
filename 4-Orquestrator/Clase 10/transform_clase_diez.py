...
df_titanic =  spark.sql("select cast(passengerId as int), cast(survived as int), cast(pclass as tinyint), cast(name as string), cast(sex as string), cast(age as int), cast(ticket as string), cast(fare as float), cast(cabin as string), cast(embarked as string) from titanic")
...
df_titanic = spark.sql("select passengerId, survived, pclass, CONCAT('\"', name, '\"') AS name, sex, age, ticket, fare, COALESCE(cabin, 0) as cabin, embarked, ROUND(avg_age_by_sex, 2) as avg_age_by_sex from columnas_eliminadas_promedio ")
